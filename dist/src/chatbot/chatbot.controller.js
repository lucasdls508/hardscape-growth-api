"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ChatbotController = void 0;
const common_1 = require("@nestjs/common");
const get_user_decorator_1 = require("../auth/decorators/get-user.decorator");
const session_auth_guard_1 = require("../auth/guards/session-auth.guard");
const meta_buisness_entity_1 = require("../page_session/entites/meta_buisness.entity");
const roles_decorator_1 = require("../user/decorators/roles.decorator");
const role_enum_1 = require("../user/enums/role.enum");
const roles_guard_1 = require("../user/guards/roles.guard");
const chatbot_service_1 = require("./chatbot.service");
const ChatRequest_dto_1 = require("./dto/ChatRequest.dto");
let ChatbotController = class ChatbotController {
    constructor(_chatbotService) {
        this._chatbotService = _chatbotService;
    }
    async handleChat(chatRequest) {
        try {
            const { clientId, userMessage, formData, userInfo } = chatRequest;
            const response = await this._chatbotService.chatWithForm(clientId, userMessage, formData, userInfo);
            return response;
        }
        catch (error) {
            throw new common_1.InternalServerErrorException({
                message: "Could not process chat message",
                detail: error.message,
            });
        }
    }
    async handleRawMessaging(chatRequest, userInfo) {
        try {
            const { clientId, userMessage } = chatRequest;
            const response = await this._chatbotService.chatRaw(clientId, userMessage, userInfo);
            return response;
        }
        catch (error) {
            throw new common_1.InternalServerErrorException({
                message: "Could not process chat message",
                detail: error.message,
            });
        }
    }
};
exports.ChatbotController = ChatbotController;
__decorate([
    (0, common_1.Post)("message"),
    (0, common_1.UsePipes)(new common_1.ValidationPipe({ transform: true })),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [ChatRequest_dto_1.ChatRequestDto]),
    __metadata("design:returntype", Promise)
], ChatbotController.prototype, "handleChat", null);
__decorate([
    (0, common_1.Post)("message/raw"),
    (0, common_1.UsePipes)(new common_1.ValidationPipe({ transform: true })),
    (0, common_1.UseGuards)(session_auth_guard_1.JwtAuthenticationGuard, roles_guard_1.RolesGuard),
    (0, roles_decorator_1.Roles)(role_enum_1.UserRoles.CONTRUCTOR, role_enum_1.UserRoles.AGENCY_OWNER),
    __param(0, (0, common_1.Body)()),
    __param(1, (0, get_user_decorator_1.GetUserInformation)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, meta_buisness_entity_1.MetaBuisnessProfiles]),
    __metadata("design:returntype", Promise)
], ChatbotController.prototype, "handleRawMessaging", null);
exports.ChatbotController = ChatbotController = __decorate([
    (0, common_1.Controller)("chatbot"),
    __metadata("design:paramtypes", [chatbot_service_1.ChatbotService])
], ChatbotController);
//# sourceMappingURL=chatbot.controller.js.map